<template>
    <div class="container">
        <h1 style="color: white;">Tipos de Problemas</h1>
        <div>
        ...
        <table class="styled-table">
            ...
            <tbody>
                <tr v-for="problema in problemas" :key="problema.id">
                    <td>{{ problema.nome }}</td>
                    <td><button @click="edit(problema)" class="button">Editar</button></td>
                    <td><button @click="remove(problema.id)" class="button">Excluir</button></td>
                </tr>
            </tbody>
        </table>
...
import { getUserFromToken } from '../../funcoes.js';
    ...
    methods: {
        ...
        async remove(id) {
            if (confirm('Tem certeza que deseja excluir este tipo de problema?')) {
                try {
                    const url = `http://localhost:8080/apis/adm/delete-tipo?id=${id}`;
                    await axios.delete(url);
                    this.problemas = this.problemas.filter(problema => problema.id !== id);
                } catch (error) {
                    alert('Falha ao excluir problema: ' + (error.response?.data?.message || error.message));
                }
            }
        },
        edit(problema) {
        ...
        async submitForm() {
            try {
                ...
                if (problema.id) {
                    const url = `http://localhost:8080/apis/adm/update-tipo`;
                    await axios.put(url, problema);
                } else {
                    const url = "http://localhost:8080/apis/adm/add-tipo";
                    await axios.post(url, problema);
                }
                this.getLista();
                this.resetForm();
            } catch (error) {
                alert('Falha ao salvar problema: ' + (error.response?.data?.message || error.message));
            }
        },
        resetForm() {
